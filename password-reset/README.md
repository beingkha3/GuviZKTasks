...
- **Generic responses** — forgot-password always returns the same message to prevent account enumerati1on.
...
| POST | `/api/auth/forgot-password` | Email a reset link (generic response, rate-limited 5/15 min) |
...
If your network or DNS blocks SRV lookups and you see `querySrv ECONNREFUSED`, use the non-SRV Atlas seed-list URI shown in `.env.example` instead of a `mongodb+srv://` URI.
